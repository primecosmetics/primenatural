function comapany_product_search(chk1) {
		
	if (chk1.srch_kword.value=="" || chk1.srch_kword.value=="Search Products") {				
		alert("Enter Search Products Keyword(s)");
...
function popup_window(url,w,h) {  
	...
	var newWin = open(url, 'win_name', 'x=0,y=0,toolbar=no,location=no,directories=no,status=no,scrollbars=yes, copyhistory=no,width='+w+',height='+h+',screenX=0,screenY=0,left=20,top=20');
...
function firefoxAlert(pageurl) {
	...
	if(navigator.appName=="Microsoft Internet Explorer") {
		...
		pageurl.style.behavior='url(#default#homepage)';pageurl.setHomePage('http://www.primenatural.us');
	}
	else {
	    alert('To set PrimeNatural.us as your home page in Firefox, click and drag the button to the "Home" icon in your browser.');
	}
	return false;
}
...
function validateTextProductCounterExt(field_id,length_field_id,maxlimit) {	
	
	document.getElementById(field_id).value = document.getElementById(field_id).value.substring(0, maxlimit);
	
	document.getElementById(length_field_id).innerHTML = maxlimit - document.getElementById(field_id).value.length;
}
...
}//func used for send inquiry
function inq_form_validate(frm) {
	
	if(frm.notkword.value=="N") {
		
		if(frm.kword.value.length=="0") {
	...
	if (frm.detail_req1.value.length==0 || frm.detail_req1.value.length<50) {
	...
	if (frm.detail_req1.value.length>1000) {
	...
	if(frm.captch_value.value!='') {
		...
		if(parseInt(frm.captcha.value)!=parseInt(frm.captch_value.value)) {
	 		...
	 		alert("Verification (Captcha Image) Code did not match with displayed code.");
	 		frm.captcha.focus();
	 		return false;
	 	}
 	}
    ...
    if (frm.login_type.value=='false') {
	    ...
	    if(frm.memberType[0].checked==false && frm.memberType[1].checked==false) {
		
			alert("Please Select Members Status");
			
	        frm.memberType[0].focus();
	        
	        return false;
	    }
    
	    //Existing member Validate
	    if (frm.memberType[0].checked && frm.memberType[0].value=="1") {
			...
			if (frm.user_name.value.indexOf('@')==-1) {
			...
			if(frm.ph_number.value=="Phone No." && frm.mobile_phone.value=="Mobile No.") {
				
				alert('Please Enter Your Phone / Mobile Number(s).');
				frm.ph_number.focus();
		     	return false;
			}
			else if(frm.ph_number.value!="Phone No.") {
								
				if (chktrim(frm.ph_number.value)=="Phone No.") {
			    ...
			    if ((frm.ph_number.value.length>8 || frm.ph_number.value.length<5) && frm.country.options[frm.country.selectedIndex].value=="IN^91") {
			    ...
			    if ((frm.ph_number.value.length>10 || frm.ph_number.value.length<5) && frm.country.options[frm.country.selectedIndex].value!="IN^91") {
			    ...
			    if (chktrim(frm.ph_acode.value).length==0 || chktrim(frm.ph_acode.value)=='STD') {
				...
				if (chktrim(frm.ph_ccode.value).length==0 || chktrim(frm.ph_ccode.value)=='ISD') {
			 ...
			 if (ph_number2Exists) {
				...
				if (frm.ph_number2.value=="") {
					   
				   alert("Please Enter Phone Number 2");
				   frm.ph_number2.focus();
				   return false;
			    }
			      
				if ((frm.ph_number2.value.length>8 || frm.ph_number2.value.length<5) && frm.country.options[frm.country.selectedIndex].value=="IN^91") {
			    ...
			    if ((frm.ph_number2.value.length>10 || frm.ph_number2.value.length<5) && frm.country.options[frm.country.selectedIndex].value!="IN^91") {
			...
			if (mobile_phone2Exists) {
				...
				if (frm.mobile_phone2.value.length<10 && frm.country.options[frm.country.selectedIndex].value=="IN^91") {
				   
				   //alert("Please Enter Valid Mobile Number 2");
				    alert("Mobile Number 2 Should be 10 Digits.");
				   frm.mobile_phone2.focus();
				   return false;
			   }
			   
			   if ((frm.mobile_phone2.value.length<3) && frm.country.options[frm.country.selectedIndex].value!="IN^91") {
			...
			if (frm.bus_ty.options[frm.bus_ty.selectedIndex].value=="")  {
		   	...
		   	if (frm.bus_ty.options[frm.bus_ty.selectedIndex].value!="x")  {
...
//this is used for ei/inquiry form
function chk_charlen_inq() {
	
	document.form1.char_len.value=document.form1.detail_req1.value.length;
	
	if (document.form1.detail_req1.value.length>1000) {
		
		alert("Your Inquiry Details Has Exceeded 1000 Characters");
		
		document.form1.detail_req1.focus();
...
//this is used for ei/inquiry form for business classified
function chk_charlen_classified() {
	
	document.form1.char_len1.value=document.form1.product_desc.value.length;
	
	if (document.form1.product_desc.value.length>350) {
		
		alert("Your Products/Services Details Has Exceeds 350 Characters");
...
function validate_form_field(val,field) {
	...
	if(field=="country" || field=="ph_number" || field=="mobile_phone" || field=="comp_name" || field=="street_address" || field=="product_desc" || field=="captcha" || field=="state_code1" || field=="your_name" || field=="detail_req1") {
...
function isProhibited(obj){
	...
	return false;
}//end of isProhibited()var xmlHttp
...
easingOut:"swing",showCloseButton:true,showNavArrows:true,enableEscapeButton:true,enableKeyboardNav:true,onStart:function(){},onCancel:function(){},onComplete:function(){},onCleanup:function(){},onClosed:function(){},onError:function(){}};b(document).ready(function(){b.fancybox.init()})})(jQuery);(function($){
